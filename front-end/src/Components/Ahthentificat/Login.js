...
class Login extends Component {
  ...
  onSubmit(e) {
    ...
    //-------------------Restaurant-----------------------
    axios
      .post("/ProjectRest/restaurant/login", restaurant)
      .then(res =>
        res.status == 200
          ? this.setState({ ...this.state, loggedRestau: true })
          : res.status(400).json({ message: "Login or Password incorrect" })
      )
      .catch(err => this.setState({ errors: err.response.data }));
  }
  ...
  render() {
    ...
    // }
    if (this.state.loggedRestau) {
      console.log("object", this.state.loggedRestau);
      return <Redirect to="/dashboardRestau" />;
    }
